...
export const createRoomReview = asyncHandler(async (req: IUserRequest, res: Response) => {
    ...
    if (room) {
        const alreadyReviewed = room.reviews?.find((review) => review?.user.toString() === req.user._id.toString());
        ...
        // Ensure that room.reviews is defined before accessing its properties
        if (room.reviews) {
            room.reviews.push(review);

            room.numOfReviews = room.reviews.length;

            room.ratings = room.reviews.reduce((acc, item) => (item?.rating || 0) + acc, 0) / room.reviews.length;

            await room.save();

            res.status(201).json({ message: 'Review added' });
        } else {
            res.status(500).json({ message: 'Internal Server Error: Room reviews array is undefined' });
        }
    } else {
        res.status(401);
        throw new Error('Room not found');
