...
const Timemoment = extendMoment(Moment as any);
...
export const getBookedDates = asyncHandler(async (req: Request, res: Response) => {
    ...
    let bookedDates: object[] = [];

    const timeDiffernece = Timemoment().utcOffset() / 60;

    bookings.forEach((booking: IBooking) => {

        const checkInDate = Timemoment(booking.checkInDate).add(timeDiffernece, 'hours')
        const checkOutDate = Timemoment(booking.checkOutDate).add(timeDiffernece, 'hours')

        const range = Timemoment.range(Timemoment(checkInDate), Timemoment(checkOutDate));
